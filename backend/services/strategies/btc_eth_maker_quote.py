"""
Strategy: BTC/ETH Maker Quote

Two-sided post_only maker quoting on BTC/ETH/SOL/XRP up-or-down markets
(5-minute, 15-minute, 1-hour, and 4-hour timeframes on Polymarket). The
strategy places post_only limit buy orders on BOTH the YES and NO sides
one tick below the current ask, earning the bid-ask spread plus maker
rebates (0% taker fee). When fresh Chainlink oracle data is available,
...
class BtcEthMakerQuoteStrategy(BaseStrategy):
    """
    ...
    name = "BTC/ETH Maker Quote"
    description = "Two-sided post_only maker quoting on BTC/ETH/SOL/XRP up-or-down markets — earns spread + maker rebates with oracle-gated inventory skew"
    ...
    def evaluate(self, signal: Any, context: dict[str, Any]) -> StrategyDecision:
        ...
        # --- Active mode (pinned to maker_quote for this strategy) ---
        active_mode = "maker_quote"
    ...
    def _detect_from_crypto_markets(self, markets: list[dict]) -> list[Opportunity]:
        ...
        for market in markets:
            ...
            def _emit_reject(verbosity: str = MURMUR, _gates=gates, _m=market) -> None:
                emit_evaluation_nowait(
                    strategy_slug="btc_eth_maker_quote",
                    market=_m,
                    gates=_gates,
                    outcome="rejected",
                    verbosity=verbosity,
