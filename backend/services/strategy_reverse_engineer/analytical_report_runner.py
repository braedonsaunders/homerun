...
async def run_analytical_report(job_id: str) -> dict[str, Any]:
    ...
    await _update(
        ...
        best_score=1.0 if analytics.headline.realized_pl_usdc is not None else 0.0,
    )
    ...
    await _persist_iteration(job_id, payload)
...
async def _persist_iteration(job_id: str, payload: dict[str, Any]) -> None:
    import time
    ...
    async with AsyncSessionLocal() as session:
        row = StrategyReverseEngineerIteration(
            ...
            score=1.0,
            score_breakdown_json={"mode": "report"},
