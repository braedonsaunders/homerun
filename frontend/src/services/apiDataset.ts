...
export interface ParquetRetentionConfig {
  enabled: boolean
...
export interface ParquetRetentionStatus {
  config: ParquetRetentionConfig
  windows_total: number
  bytes_total: number
}

export interface ParquetPrunePreview {
  windows_total: number
  bytes_total: number
  windows_to_delete: number
...
export interface ParquetPruneResult {
  windows_deleted: number
  bytes_freed: number
  catalog_rows_pruned: number
  duration_seconds: number
  max_age_days: number | null
  max_total_gb: number | null
}

export async function getParquetRetention(): Promise<ParquetRetentionStatus> {
  const { data } = await api.get<ParquetRetentionStatus>('/dataset/storage/retention')
  return data
}

export async function updateParquetRetention(
  patch: Partial<ParquetRetentionConfig>,
): Promise<{ config: ParquetRetentionConfig }> {
  const { data } = await api.put<{ config: ParquetRetentionConfig }>('/dataset/storage/retention', patch)
  return data
}

export async function previewParquetPrune(
  params: { max_age_days?: number; max_total_gb?: number } = {},
): Promise<ParquetPrunePreview> {
  const { data } = await api.post<ParquetPrunePreview>('/dataset/storage/prune', null, {
    params: { execute: false, ...params },
  })
  return data
}

export async function executeParquetPrune(
  params: { max_age_days?: number; max_total_gb?: number } = {},
): Promise<ParquetPruneResult> {
  const { data } = await api.post<ParquetPruneResult>('/dataset/storage/prune', null, {
    params: { execute: true, ...params },
  })
  return data
}
